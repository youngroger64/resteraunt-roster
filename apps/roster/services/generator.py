...
def candidate_availability(roster, employee, shift_date, signature):
    ...
    if availability_exception:
        if availability_exception.unavailable:
            return {
                "available": False,
                "possible_split": False,
                "reason": "Employee marked unavailable",
            }

        if (
            availability_exception.available_from
            and availability_exception.available_until
        ):
            window_start, window_end = _interval(
                shift_date,
                availability_exception.available_from,
                availability_exception.available_until,
            )
            for proposed_start, proposed_end in proposed:
                if proposed_start < window_start or proposed_end > window_end:
                    return {
                        "available": False,
                        "possible_split": False,
                        "reason": (
                            "Available only "
                            f"{availability_exception.available_from.strftime('%H:%M')}–"
                            f"{availability_exception.available_until.strftime('%H:%M')}"
                        ),
                    }
    ...
    if not existing:
        return {
            ...
            "possible_split": False,
            "reason": (
                "Matches submitted availability"
                if availability_exception
                else "Available for this shift"
            ),
        }
...
def target_days(pattern):
    profile = getattr(pattern.employee, "schedule_profile", None)
    if profile and profile.target_days:
        return min(7, max(0, int(profile.target_days)))
    return min(7, max(0, math.ceil(float(pattern.average_days_worked))))



def target_hours(pattern, department=None):
    # Manager-approved scheduling profiles override learned averages. Historic
    # patterns remain the fallback for employees without an explicit profile.
    profile = getattr(pattern.employee, "schedule_profile", None)
    if profile and float(profile.target_hours or 0) > 0:
        preferred = profile.preferred_department or pattern.normal_department or pattern.employee.department
        if department is None or department == preferred:
            return float(profile.target_hours)
...
def automatic_department_eligible(pattern, department):
    ...
    # A manager-approved main area is policy, not a soft preference.
    # Cross-trained employees may still be manually moved, but automatic
    # generation should fill their primary area first and predictably.
    profile = getattr(employee, "schedule_profile", None)
    if profile and profile.preferred_department:
        return profile.preferred_department == department
...
def automatic_assignment_safe(
    ...
    """
    Decide whether the generator may automatically assign this employee.

    Ranking is deliberately softer: an over-target employee can still appear
    as a manager suggestion. Automatic generation, however, must respect the
    employee's normal working-day and weekly-hours limits.
    """

    day_limit = target_days(pattern)

    # If this shift would create another working day beyond the employee's
    # normal target, do not assign it automatically.
    if day_limit and current_days >= day_limit:
        return False

    proposed_hours = signature_duration(signature)
    projected_hours = current_hours + proposed_hours

    # The existing ceiling includes the allowed tolerance above the employee's
    # normal weekly hours.
    ceiling = automatic_hour_ceiling(pattern, department)
...
def create_assignment_or_open(
    ...
    open_shift = OpenShift.objects.create(
        ...
        notes="No safe eligible employee available",
    )
